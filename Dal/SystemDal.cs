...
namespace TD.Dal
{
    ...
    public class SystemDal
    {
        ...
        public SystemDal(string connStr)
        {
        ...
        {
            string sql = "select * from t_system where system_id=@system_id";
            using (var conn = new Npgsql.NpgsqlConnection(ConnectionString))
            {
                return conn.QueryFirstOrDefault<SystemModel>(sql, new { system_id = system_id });
            }
        }
        /// <summary>
        /// 获取数据列表
        ...
        {
            string sql = "select * from t_system where 1=1 " + GetCondition(keyword);
            using (var conn = new Npgsql.NpgsqlConnection(ConnectionString))
            {
                return conn.Query<SystemModel>(sql, new { keyword = "%" + keyword + "%" }).AsList();
            }
        }
       /// <summary>
       /// 获取分页数据
       ...
       /// <returns></returns>
        public List<SystemModel> GetPager(string keyword, int pageSize, int pageNo)
        {
            string sql = DalHelper.GetPagerSql("t_system", "*", " system_name asc", GetCondition(keyword), pageSize, pageNo);
            using (var conn = new Npgsql.NpgsqlConnection(ConnectionString))
            {
        ...
        {
            string sql = "select count(1) from t_system where 1=1 " + GetCondition(keyword);
            using (var conn = new Npgsql.NpgsqlConnection(ConnectionString))
            {
        ...
        public string GetCondition(string keyword)
        {
            ...
            {
                result.Append(" and system_name like @keyword");
            }
